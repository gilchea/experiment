"""
plot_rcv1_convex.py — Trực quan hóa kết quả cho Rcv1 Convex
"""
...
RESULTS_FILE = os.path.join(RESULTS_DIR, 'rcv1_convex_results.json')
...
def plot_loss_residual(data):
    ...
    ax.set_title('Rcv1 Convex: Training Loss Residual')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    os.makedirs(FIGURES_DIR, exist_ok=True)
    out_path = os.path.join(FIGURES_DIR, 'rcv1_convex_loss.png')
    plt.tight_layout()
...
def plot_variance(data):
    ...
    ax.set_title('Rcv1 Convex: Gradient Variance')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    out_path = os.path.join(FIGURES_DIR, 'rcv1_convex_variance.png')
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
    plt.close()
...
def plot_training_loss(data):
    ...
    ax.set_title('Rcv1 Convex: Training Loss')
    # ax.set_ylim(0.26, 0.33)
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    os.makedirs(FIGURES_DIR, exist_ok=True)
    out_path = os.path.join(FIGURES_DIR, 'rcv1_convex_training_loss.png')
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
    plt.close()
...
def main():
    if not os.path.exists(RESULTS_FILE):
        print(f"Cannot find {RESULTS_FILE}. Please run merge_covtype_convex.py first.")
        return
